...
import '../../widgets/product/home_product_list.dart';
...
class HomeProductsPage extends StatefulWidget {
  ...
  State<StatefulWidget> createState() {
    return HomeProductsPageState();
  }
}

class HomeProductsPageState extends State<HomeProductsPage>
...
//  int attributeTerm;
  ...
  void _startLoadingTimer() {
    ...
    _loadingTimer = Timer.periodic(interval, (_) async {
      if(mounted) {
        if (!loadProducts && Provider.of<HomeProductModel>(context, listen: false).productsList!.isNotEmpty &&
            !Provider.of<HomeProductModel>(context, listen: false).isEnd!) {
          await onLoadMore();
        }
      }
    });
  ...
  Future loadCategoriesList(
      List categoryIdList, HomeProductModel productModel, tagId, terms) async {
  ...
  Future<void> onFilter(
    ...
    final productModel = Provider.of<HomeProductModel>(context, listen: false);
    final filterAttr =
  ...
  void onSort(order) {
    ...
    final productModelT = Provider.of<HomeProductModel>(context, listen: false);
  ...
  Widget _renderItemCategory({String? categoryId, required String categoryName}) {
    return GestureDetector(
      ...
      onTap: () {
        print("running");
        _page = 1;
        if(!Provider.of<HomeProductModel>(context, listen: false).isFetching) {
          Provider.of<HomeProductModel>(context, listen: false).setProductsList(<Product>[]);
          Provider.of<HomeProductModel>(context, listen: false).setCurrentCat(categoryId ?? "");
  ...
  Future<void> onLoadMore() async {
    ...
    if(newCategoryIdList![0] != "") {
      newCategoryIdList!.forEach((catId) async {
        if(isLoading) return;
        await Provider.of<HomeProductModel>(context, listen: false).getMoreProducts(
            categoryId: catId,
            minPrice: minPrice,
            maxPrice: maxPrice,
            lang: Provider.of<AppModel>(context, listen: false).langCode,
      ...
      if(isLoading) return;
      await Provider.of<HomeProductModel>(context, listen: false).getProductsBySearchValue(
        ...
        builder: (context, constraint) {
          return FractionallySizedBox(
            ...
            child: ListenableProvider.value(
              value: product,
              child: Consumer<HomeProductModel>(builder: (context, value, child) {
                return backdrop(
