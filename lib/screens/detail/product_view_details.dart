...
class ProductTitleView extends StatefulWidget {
  ...
  ProductTitleView(this.product, this.value);

  @override
  _ProductTitleState createState() => _ProductTitleState();
}

class _ProductTitleState extends BaseScreen<ProductTitleView> {
  var regularPrice;
  bool? onSale = false;
  int sale = 100;
  String? price;
        ...
        Row(
          children: <Widget>[
            widget.product.type == 'configurable'
                ? Text(
                    "${Tools.getCurrencyFormatted(price ?? "0.0", currencyRate, currency: currency)! + "  " + "Per" + "  " + widget.value}",
                    style: Theme.of(context).textTheme.titleMedium!.copyWith(
                          fontSize: 14,
                          fontWeight: FontWeight.w600,
