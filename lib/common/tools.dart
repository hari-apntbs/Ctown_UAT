...
class Tools {
  static double? formatDouble(dynamic value) =>
      value is String ? double.parse(value) : value * 1.0;
  ...
  static Widget image({
        ...
        children: <Widget>[
          Container(
            ...
            child: CachedNetworkImage(
              imageUrl: isResize ? formatImage(url, size)! : url,
              width: width,
              height: height ?? width! * 1.2,
              fit: fit,
              placeholder: (context, url) => hidePlaceHolder
                  ? const SizedBox()
                  : Skeleton(
                width: width ?? 100,
                height: height ?? 140,
    ...
    //     );
    //   },
    //   cacheManager: CustomCacheManager.instance, // Use a custom cache manager
    // );
    return ExtendedImage.network(
      ...
      cacheMaxAge: Duration(minutes: 2),
      alignment: Alignment(
        (offset >= -1 && offset <= 1)
            ? offset
            : (offset > 0)
  ...
  static String? getPriceProductValue(product, String? currency, {bool? onSale}) {
    try {
      String? price = product.onSale == true
          ? (isNotBlank(product.salePrice)
              ? double.parse(product.price) > double.parse(product.salePrice)
                  ? product.salePrice
                  : product.price ?? '0'
              : product.price)
          : (isNotBlank(product.regularPrice)
              ...
              : product.price);
      printLog("Price Testing");
      printLog(product.regularPrice);
      printLog(product.price);
      printLog(product.onSale);
      return price;
    } catch (e, trace) {
      printLog(e.toString());
  ...
  static showSnackBar(ScaffoldMessengerState scaffoldState, message) {
...
class Utils {
  ...
  static onTapNavigateOptions(
    ...
    //     context,
    //     MaterialPageRoute<void>(
    //       builder: (BuildContext context) => BlogView(id: config['blog'].toString()),
    //       fullscreenDialog: true,
    //     ),
    //   );
    // }
    ...
    //     context,
    //     MaterialPageRoute<void>(
    //       builder: (BuildContext context) => BlogListView(id: config['blog_category'].toString()),
    //       fullscreenDialog: true,
    //     ),
    //   );
    // }
