    ...
    switch (Config().type) {
      ...
      case "strapi":
        ...
        for (var item in _jsons) {
          _blogs.add(Blog.fromStrapiJson(item));
        }
        blogs = [..._blogs as Iterable<Blog>];
        break;
      default:
...
class Blog {
  ...
  static Future<dynamic> getBlogs({url, categories, page = 1}) async {
    ...
    final response = await http.get(Uri.parse("$url/wp-json/wp/v2/posts?$param"));
    return jsonDecode(response.body);
  }

  static Future<dynamic> getBlog({url, id}) async {
    final response = await http.get(Uri.parse("$url/wp-json/wp/v2/posts/$id?_embed"));
    return jsonDecode(response.body);
  }

  static Future<dynamic> getBlogsFromStrapi({url}) async {
    final response = await await http.get(Uri.parse("$url/posts"));
    return jsonDecode(response.body);
  }
