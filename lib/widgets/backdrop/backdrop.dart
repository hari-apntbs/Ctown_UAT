...
import '../../screens/categories/brand_filter.dart';
...
class _BackdropState extends State<Backdrop>
  ...
  List? data = [];
  List sort = ["position", "name", "price"];
  List sortar = ["موقع", "اسم", "السعر"];
  List tt = [];
  ...
  brandfliter() async {
    Uri uri = Uri.parse("https://up.ctown.jo/api/brandfilter.php");
    String? id = widget.product;
    var response = await http.post(uri,
        body: json.encode({"category_id": widget.product}));
    print("#############################################################");
    print(widget.product);
    print({"category_id": widget.product});
    ...
    return PopupMenuItem<String>(
      ...
      child: Row(
        children: <Widget>[
          ...
          Padding(
            padding: const EdgeInsets.only(right: 10.0),
            child: Text(label, style: menuItemStyle),
          ),
        ],
      ),
    );
  ...
  Widget build(BuildContext context) {
    ...
    var appBar = AppBar(
      ...
      actions: <Widget>[
        ...
        //   ),
        //   onPressed: () {
        //     // return Navigator.of(context).pushNamed(RouteList.homeSearch, arguments: '');\
        ...
        PopupMenuButton(
            ...
            onSelected: (dynamic value) {
              ...
              // Navigator.of(context).push(MaterialPageRoute(
              //     builder: (context) =>  ProductModel.showList(

              //     )));
              print(widget.product);
              // ProductModel.showList(
              //     context: context,
              //     cateId: widget.product,
              //     cateName: "",
              //     sort: _mySelection,
              // );
              ProductModel.sortList(context, _mySelection);
            },
            itemBuilder: (context) {
              return Provider.of<AppModel>(context, listen: false).langCode ==
                      'en'
                  ? sort.map((choice) {
                      return PopupMenuItem(
                        value: choice,
