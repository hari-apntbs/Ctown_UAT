...
class MagentoHelper {
  ...
  static String buildUrl(String? domain, String? endpoint, String? storeCode) {
    // List<Map<String, dynamic>> languages = Utils.getLanguagesList();
    // if (isNotBlank(locale)) {
    //   Map<String, dynamic> language = languages.firstWhere(
    //           (o) => o["code"] == locale && isNotBlank(o["storeViewCode"]),
    //       orElse: () => null);
    if (storeCode != null && storeCode.isNotEmpty) {
      printLog("cvxfgdfghdfghf");
      // return "$domain/index.php/rest/${language["storeViewCode"]}/V1/$endpoint";
      return "$domain/index.php/rest/$storeCode/V1/$endpoint";
      // return "$domain/index.php/rest/qatar_al_mustah_ar/V1/$endpoint";
    }
    // }
    return "$domain/index.php/rest/V1/$endpoint";
  }

  static String buildfetchProductsByCategoryUrl(
      String domain, String storeCode, String endpoint,
      [String? locale]) {
    List<Map<String, dynamic>>? languages = Utils.getLanguagesList();
    if (isNotBlank(locale)) {
      Map<String, dynamic>? language = languages!.firstWhereOrNull(
          (o) => o["code"] == locale && isNotBlank(o["storeViewCode"]));
      if (language != null) {
        // return "$domain/index.php/rest/${language["storeViewCode"]}/V1/$endpoint";
  ...
  static String buildCategoryUrl(
      String? domain, String storeCode, String? endpoint,
      [String? locale]) {
    List<Map<String, dynamic>>? languages = Utils.getLanguagesList();
    if (isNotBlank(locale)) {
  ...
  static bool isEndLoadMore(body) {
    ...
    int maxPage = (total_count / page_size).ceil();
